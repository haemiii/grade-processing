...
def cutter2(img):    
    ...
    img_trim = cv2.imread("./grade-process/test/picture/cut_image_0.png")
...
# 높이를 문항 개수로 나누고 잘라 배열에 저장
    ...
    for i in range(20):
        cv2.imwrite(f'./grade-process/test/picture2/cut_image_{i}.png', result[i])
    cv2.imwrite("./grade-process/test/picture2/result.png", 255-img_trim)
